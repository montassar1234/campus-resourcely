...
export const Route = createFileRoute("/tags")({
  head: () => ({ meta: [{ title: "Tags — Campus Resource Hub" }] }),
  component: TagsPage,
});
    ...
    <PageShell title="Tags" subtitle="Group resources by category">
      ...
      {isLoading ? (
        <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-4">
          {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-24 animate-pulse rounded-xl border border-border bg-card/60" />)}
        </div>
      ) : (data ?? []).length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-4">
          {data!.map((t) => (
            <div key={t.id} className="group flex items-center justify-between rounded-xl border border-border bg-card p-4 shadow-soft transition hover:-translate-y-0.5 hover:shadow-elevated">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-accent/15 text-accent"><TagIcon className="h-4 w-4" /></div>
                <div>
                  <div className="font-medium">{t.name}</div>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground"><Hash className="h-3 w-3" />{t.resourceCount ?? 0} resources</div>
                </div>
              </div>
              <div className="flex gap-1 opacity-0 transition group-hover:opacity-100">
                <Button size="icon" variant="ghost" onClick={() => { setEditing(t); setDialogOpen(true); }}><Pencil className="h-3.5 w-3.5" /></Button>
                <Button size="icon" variant="ghost" onClick={() => setConfirmId(t.id)} className="text-destructive hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button>
              </div>
            </div>
          ))}
        </div>
...
function TagDialog({ open, onOpenChange, editing }: { open: boolean; onOpenChange: (o: boolean) => void; editing: Tag | null }) {
  const qc = useQueryClient();
  const { register, handleSubmit, reset, formState: { errors } } = useForm<{ name: string }>({
    values: { name: editing?.name ?? "" },
  });
  const mut = useMutation({
    mutationFn: (v: { name: string }) => editing ? api.tags.update(editing.id, v) : api.tags.create(v),
    onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle className="font-display">{editing ? "Edit tag" : "New tag"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit((v) => mut.mutate(v))} className="space-y-4">
          <div className="space-y-1.5">
            <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Name</Label>
            <Input {...register("name", { required: "Required", maxLength: { value: 50, message: "Max 50 chars" } })} placeholder="e.g. Camera, Electronics" />
